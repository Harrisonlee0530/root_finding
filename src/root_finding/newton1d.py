...
def newton1d(f: Callable[[float], float], df: Callable[[float], float], x0: float, tol1: float):
    r"""
    ...
    Notes
    -----
    The Newton–Raphson method updates the current estimate using the local linear
    approximation of `f` at the current point:

    ``x_{n+1} = x_n - f(x_n) / df(x_n)``.

    The iteration stops when the relative change in successive iterates is below
    the specified tolerance `tol1`, i.e., when:

    ``| (x_{n+1} - x_n) / x_{n+1} | < tol1``.

    This method often converges rapidly when `x0` is sufficiently close to the
    true root and `df(x)` is well-behaved, but it may fail when `df(x_n) = 0` or
